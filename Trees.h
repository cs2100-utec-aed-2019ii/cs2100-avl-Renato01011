...
public:
	...
	bool CheckIfTwoTreesAreEqual(AVLTree<T> bt) {
	...
	void CheckBalance(Node<T>* toCheckNode) {
		if (toCheckNode == nullptr) { return; }
		Node<T>* parent = ReturnParent(toCheckNode);
		if (parent == nullptr) { return; }
		Node<T>* gparent = ReturnParent(parent);
		if (gparent == nullptr) { return; }
		int balance_val_gparent = Height(gparent->right) - Height(gparent->left);
		if (balance_val_gparent > 1) {
			if (gparent->right->right == toCheckNode) {
				Node<T>* temp = parent->left;
				Node<T>* ggparent = ReturnParent(gparent);
				if (ggparent != nullptr) {
			...
			else if (gparent->right->left == toCheckNode) {
				Node<T>* temp = toCheckNode->right;
				toCheckNode->right = parent;
				parent->left = temp;
				gparent->right = toCheckNode;
				CheckBalance(parent);
			}
		}
		else if (balance_val_gparent < -1) {
			if (gparent->left->left == toCheckNode) {
				std::cout << "Im here bruh";
			...
			else if (gparent->left->right == toCheckNode) {
				Node<T>* temp = toCheckNode->left;
				toCheckNode->left = parent;
				parent->right = temp;
				gparent->left = toCheckNode;
				CheckBalance(parent);
			}
		}
		CheckBalance(parent);
